...
export async function setMultipleSettings(settings: Record<string, any>): Promise<boolean> {
  ...
  for (const [key, value] of Object.entries(settings)) {
    ...
    if (existing) {
      ...
      await client.from('site_settings').insert({ ...payload, id: crypto.randomUUID() })
    }
  }
  return true
}
...
export async function saveLastWeekHighlight(highlight: any): Promise<boolean> {
  ...
  if (existing) {
    ...
    await client.from('last_week_highlights').insert({ ...payload, id: crypto.randomUUID() })
  }
  return true
}
...
export async function savePopup(popup: any): Promise<boolean> {
  ...
  if (existing) {
    ...
    await client.from('popup').insert({ ...payload, id: crypto.randomUUID() })
  }
  return true
}
...
export async function saveAnnouncement(announcement: any): Promise<boolean> {
  ...
  if (existing) {
    ...
    await client.from('announcement').insert({ ...payload, id: crypto.randomUUID() })
  }
  return true
}
